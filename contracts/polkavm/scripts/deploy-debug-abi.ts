...
const POLKAVM_PATH = resolve(__dirname, '../target/forms_debug_abi.polkavm');
const OUTPUT_PATH = resolve(__dirname, '../deployment_debug_abi.json');
...
async function deploy() {
  console.log('\n🧪 Deploying Debug ABI Parsing Test Contract');
  console.log('============================================================\n');
  ...
  if (!existsSync(POLKAVM_PATH)) {
    console.error('❌ forms_debug_abi.polkavm not found. Run: cargo build --release --bin forms_debug_abi && polkatool link --strip --output target/forms_debug_abi.polkavm target/riscv64emac-unknown-none-polkavm/release/forms_debug_abi\n');
    process.exit(1);
  }
